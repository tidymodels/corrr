...
#' @export
cor_matrix.matrix <- function(x, use = "pairwise.complete.obs",
...
#' @export
cor_frame.matrix <- function(x, use = "pairwise.complete.obs",
                              method = c("pearson", "kendall", "spearman")) {
  cor_frame(as.data.frame(x), use = use, method = method)
}
